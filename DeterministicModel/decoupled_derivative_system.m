...
%}
dy(1) = 4*parameters.f0 - 4*((parameters.Vmax.*O2)./(parameters.Km.*(1+(parameters.K1./Cytcred))+O2)).*Hn; %dCytcred
dy(2) = -((parameters.Vmax.*O2)./(parameters.Km.*(1+(parameters.K1./Cytcred))+O2)).*Hn; %dO2
dy(3) = -12*parameters.f0 - 8*((parameters.Vmax.*O2)./(parameters.Km.*(1+(parameters.K1./Cytcred))+O2)).*Hn ...
    + ((parameters.p1*(Hp/Hn))/((Hp/Hn)+parameters.p2+(parameters.p3/Hn)))*Hp; %dHn
dy(4) = 12*parameters.f0 + 4*((parameters.Vmax.*O2)./(parameters.Km.*(1+(parameters.K1./Cytcred))+O2)).*Hn ...
