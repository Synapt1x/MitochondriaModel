...
%This function maintains all the derivatives relevant to the project
    ...
    %Equations are written out in their entirety here, with alterations
    %taking place in these equations if the condition changes (i.e.,
    %baseline vs oligo vs FCCP vs inhibit)
    dy(1) = 2*parameters.f0 - 4*((parameters.Vmax*s_2)./(parameters.Km.*(1+(parameters.K1))+s_2)).*Hn; %dCytcred
    dy(2) = -((parameters.Vmax*s_2)./(parameters.Km.*(1+(parameters.K1))+s_2)).*Hn; %dO2
    dy(3) = -6*parameters.f0 - 8*((parameters.Vmax*s_2)./(parameters.Km.*(1+(parameters.K1))+s_2)).*Hn ...
       + ((parameters.p1*(Hp/Hn))/((Hp/Hn)+parameters.p2+(parameters.p3/Hn)))*Hp; %dHn
    dy(4) = 8*parameters.f0 + 4*((parameters.Vmax*s_2)./(parameters.Km.*(1+(parameters.K1))+s_2)).*Hn ...
