...
% handles    structure with handles and user data (see GUIDATA)
OptimalSolutions=getGUIData('OptimalSolutions');
points=get(handles.pointList, 'String');
value=get(handles.pointList, 'Value');
if ~isempty(points) && ~isempty(value) &&...
        value >= 1 && value <= length(points)
    thisPoint=str2num(points{value});
    points(value)=[];
    set(handles.pointList, 'String', points);
    NPoints=length(points);
    if value > NPoints
        set(handles.pointList, 'Value', NPoints);
    end
end
if isnumeric(thisPoint)
    NColourGroups=length(OptimalSolutions.paintedPoints.index);
    for c=1:NColourGroups
        indexPoint=OptimalSolutions.paintedPoints.index{c} == thisPoint;
        if any(indexPoint)
            OptimalSolutions.paintedPoints.index{c}(indexPoint)=[];
            OptimalSolutions.paintedPoints.X{c}(:,indexPoint)=[];
            OptimalSolutions.paintedPoints.F{c}(:,indexPoint)=[];
            OptimalSolutions.paintedPoints.rank{c}(indexPoint)=[];
            if ~isempty(OptimalSolutions.paintedPoints.auxOutput)
                OptimalSolutions.paintedPoints.auxOutput{c}{indexPoint}=[];
            end
            if isempty(OptimalSolutions.paintedPoints.index{c})
                OptimalSolutions.paintedPoints.index(c)=[];
                OptimalSolutions.paintedPoints.X(c)=[];
                OptimalSolutions.paintedPoints.F(c)=[];
                OptimalSolutions.paintedPoints.rank(c)=[];
                OptimalSolutions.paintedPoints.colour(c,:)=[];
                if ~isempty(OptimalSolutions.paintedPoints.auxOutput)
                    OptimalSolutions.paintedPoints.auxOutput(c)=[];
                end
            end
            break
        end
...
setGUIData('OptimalSolutions', OptimalSolutions);
buildPaintedPointList(handles.pointList, handles);
plotFinalResults;
...
OptimalSolutions=getGUIData('OptimalSolutions');
if ~isempty(OptimalSolutions) && isfield(OptimalSolutions, 'paintedPoints')
    OptimalSolutions=rmfield(OptimalSolutions, 'paintedPoints');
end
setGUIData('OptimalSolutions', OptimalSolutions);
buildPaintedPointList(handles.pointList, handles);
plotFinalResults;
