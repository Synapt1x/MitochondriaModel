...
if mod(world.par.bookKeeping.gen, NRefine) == 0
    NPop=length(world.pop);
    NGenes=size(world.pop{1}.indiv.genome.XPhys,1);
    Z=zeros(NPop,1);
    world.par.general.min=[world.par.general.min, Z, Z];
    world.par.general.max=[world.par.general.max, Z+1, Z+2*pi];
    ...
    for p=1:NPop
        ...
        N=size(world.pop{p}.indiv.genome.XPhys,2);
