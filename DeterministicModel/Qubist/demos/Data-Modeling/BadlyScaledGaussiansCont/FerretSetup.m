...
continuum.min=[0,0.1,0]; % [height, width, centre]
continuum.max=[1,1,1]; % [height, width, centre]
minHeight=1e-6; minWidth=1e-2;
ONE=ones(1,NTerms); ZERO=zeros(1,NTerms);
if par.user.extPar.fixCentre
    par.general.min=[minHeight+ZERO, minWidth+ZERO];
    par.general.max=[ONE, maxWidth+ZERO];
else
    par.general.min=[minHeight+ZERO, minWidth+ZERO, ZERO];
    par.general.max=[ONE, maxWidth+ZERO, ONE];
end
% Add continuum.
par.general.min=[par.general.min, continuum.min];
par.general.max=[par.general.max, continuum.max];
%
% More GA general parameters.
